...
public class TestBase {
	private static final String PATH_TO_TEST_CAPS_JSON = "src/test/resources/test_caps.json";
	private static final ThreadLocal<AppiumDriver<?>> driver = new ThreadLocal();
	private static final String BROWSERSTACK_HUB_URL = "hub-cloud.browserstack.com";
	private static final long TIMESTAMP = (new Date()).getTime();
	private static final String appLocation = FileSystems.getDefault()
			.getPath(System.getProperty("user.dir"), "/src/test/resources/app").toString();
	private Local local;
	...
	public void setUp(@Optional("remote") String environment, @Optional("single") String testType,
			@Optional("0") int env_cap_id, Method m) throws Exception {
		...
		if (environment.equalsIgnoreCase("on-prem")) {
			...
			capabilities.setCapability("app",
					appLocation.concat(File.separator).concat(testCapsConfig.get("app.local").toString()));
			...
			String app = envCapabilities.get("platformName").equalsIgnoreCase("android")
					? testCapsConfig.get("app.android").toString()
					: testCapsConfig.get("app.ios").toString();
			commonCapabilities.put("app", app);
			//commonCapabilities.put("name", Scenario.this.getName());
			...
			driver.set(new AppiumDriver(
					new URL("https://" + username + ":" + accessKey + "@" + BROWSERSTACK_HUB_URL + "/wd/hub"), caps));
